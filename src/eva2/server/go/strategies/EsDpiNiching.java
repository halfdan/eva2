...
import eva2.server.go.operators.cluster.ClusteringDynPeakIdent;
...
/**
 * The ES niching with dynamic peak identification. Basically, for a set of "peaks" a set of (mu,lambda)-ESes are
 * performed in parallel, with interaction based on a niche radius (fixed so far). 
 * The number of peaks must be predefined, and lambda new samples are drawn for every peak in every iteration.
 * Thus, in relation to the standard ES, some new parameters are due. On the other hand, the selection schemes
 * are predefined. This is done according to Shir&Bäck, Niching in Evolution Strategies, Tec.Report 2005.
 * 
 * Unfortunately the algorithm was not described in every detail. It remained unclear how exactly the
 * peak population where filled and esp. what happens if DPI returns less than q peaks.
 * In this implementation, this is solved by setting a maximum size to each peak cluster (muPerPeak). For
 * a peak population, only the best muPerPeak remain and the rest is assigned to the unclustered set. From these,
 * pseudo-peaks are then formed until the expected number of peak populations is produced.
 * 
 * Also, they gave a general rule for setting the niche radius depending on the problem domain, however in their
 * experiments, they were able to identify optima which were much closer (i.e., on Ackley's), so it is unclear
 * which niche radius was actually used there
 * 
 * Due to the "non-standard" environmental selection mechanism (a fixed proportion is chosen from the children, the
 * rest from the parents), a preselection mechanism was implemented here, slightly breaking the ES framework
 * which is still used to optimize the single peak populations.
 * 
 * This class should also cover "Dynamic niching in evolution strategies with covariance matrix adaptation" 
 * by Shir & Bäck, CEC 2005, when employing SelectBest as parent selection and muPerPeak=1.
 * 
 * Some notes: 
 * 
 * If strictNiching is deactivated, niches are disappearing because they are sometimes 
 * assigned the global opt. and in that niche are very bad so they get thrown away due to the maximum niche count
 * if strictNiching is activated, niches are very small - or even empty. Why is that? This can hardly be sensible 
 * esp. with a very small niche radius for F8. This would mean that in the beginning, nearly all offspring 
 * are created from very few pairs of peaks.
 * 
 * The mu/lambda information was lost on the way to the MutateESRankMuCMA class... I added it now as additional pop.data 
 * which is only loaded if the mu/lambda ratio deduced from the orig/selected population sizes does not make sense.
 * 
 * With the explorer peaks reinited every 100 generations or so, the course of the MPR for a *single* run
 * will not be a clear logistic function, because "finding" additional peaks produces small steps in the MPR.
 * 
 * The Niching ES is now using the Population to catch the generational events, not producing it itself, similar
 * to Tribes, ScatterSearch or IPOP-ES. Thus it could be used with a dynamic population size without too much
 * hassle. 
 *
 * TODO
 * Add adaptive niche radius. Add parameter to every indy which is adapted after all new peaks have been found.
 *  
 * 
 * @author mkron
 *
...
public class EsDpiNiching implements InterfaceOptimizer, Serializable, InterfaceAdditionalPopulationInformer, InterfacePopulationChangedEventListener {
	...
	/**
	 * Use the given parameters, the maximal estimation for the niche radius, no preselection and no explorers.
	 * 
	 * @param muPerPeak
	 * @param lambdaPerPeak
	 * @param expectedPeaks
	...
	/**
	 * If the niche radius is zero or negative, the automatic estimation is used. Otherwise the given radius is used.
	 * If etaPresel is positive, eta-preselection is activated, otherwise it is inactive.
	 * 
	 * @param nicheRadius
	 * @param muPerPeak
	 * @param lambdaPerPeak
	...
	public EsDpiNiching(double nicheRadius, int muPerPeak, int lambdaPerPeak, int expectedPeaks, int rndImmigrants, int explorerPeaks, int resetExplInterval, int etaPresel) {
		...
		} else setUseNicheRadiusEstimation(true);
		this.eta=etaPresel;
	...
	public EsDpiNiching(EsDpiNiching o) {
		...
		if (o.population!=null) this.population = (Population)o.population.clone();	
		if (o.problem!=null) this.problem = (InterfaceOptimizationProblem)o.problem.clone();
...
//		int initialImmigrants = (fullNumPeaks()*lambdaPerPeak+getNumRndImmigrants())-(fullNumPeaks()*muPerPeak+getNumRndImmigrants());
    	...
    	if (isUseNicheRadiusEstimation()) updateNicheRadius();
	}
	...
	private void updateNicheRadius() {
		...
		} else System.err.println("Error, default niche radius can only be estimated for individuals of type InterfaceDataTypeDouble!");
	}

	/**
	 * Use the deactivation of converged species, storing them to an archive,
	 * with the given parameters. If windowLen <= 0, the deactivation 
	...
	/**
	 * Calculate the estimated maximal niche radius for a given problem range. This
	 * is an estimate on the q-th part of the volume transfered to the radius of a hypersphere,
	 * where q is the number of expected peaks.
	 *  
	 * @param range
	...
	public static double calcEstimatedNicheRadius(double[][] range, int numExpectedOptima, InterfaceDistanceMetric metric) {
		...
		if (metric instanceof EuclideanMetric) {
			...
			if (!(metric instanceof PhenotypeMetric)) {
				if ((metric instanceof IndividualDataMetric) && (((IndividualDataMetric)metric).getBaseMetric() instanceof EuclideanMetric)) {} // this is ok as well
				else System.err.println("Warning, unexpected metric in EsDpiNiching! Estimated niche radius may fail...");
			}
			r = 0.5*Math.sqrt(dim);
		} 
//		setNicheRadius(r*Math.pow(numExpectedPeaks, -1./(double)dim));
	...
	public void addExpectedPeaks(int k) {
		...
		if (k<=0) System.err.println("Invalid k in addExpectedPeaks (" + k + ").");
		...
		for (int i=0; i<newPeakOpts.length; i++) { // allocate new peak optimizers
			if (i<peakOpts.length) newPeakOpts[i]=peakOpts[i];
			else newPeakOpts[i] = new EvolutionStrategies(peakOpts[0]);
		}
...
//		else return 0;
	...
	public void optimize() {
		...
		if (TRACE) System.out.println("--- FULL POP SIZE: " + population.size() + " , funcalls: " + population.getFunctionCalls() + ", gen " + population.getGeneration());
		...
		for (int i=0; i<peakPopSet.length; i++) peakPopSet[i].removePopulationChangedEventListener(this);
		setGeneration(population.getGeneration(), peakPopSet);
		if (TRACE_DEMES) printDemes("After clustering: ", peakPopSet);
...
//		}
		...
		int reqNewPeaks = 0;
        if (curNumPeaks<getExpectedPeaks()) reqNewPeaks = getExpectedPeaks()-curNumPeaks; // not enough desired peaks were found
        if (getExplorerPeaks()>0) {
        	if ((getPopulation().getGeneration() % resetExplorerInterval == 0)) {
        		...
        		for (int i=0; i<clustersWithExplorers.length; i++) {
        			if (i<peakPopSet.length) clustersWithExplorers[i]=peakPopSet[i];
        			// index i-1 because unclustered indies do not have an optimizer 
        			else clustersWithExplorers[i] = peakOpts[i-1].getPopulation();
        		}
		...
		if (TRACE_DEMES) printDemes("After expansion: ", peakPopSet);
        ...
        if (archive!=null && (archive.size()>0) && isReinitOnCollision()) {
        	...
        	for (int i=0; i<assoc.length; i++) {
        		if (assoc[i]>=0) {
        			...
        			if (TRACE) System.out.println(" Converged on archived solution.. resetting peak pop " + assoc[i]);
        			peakPopSet[assoc[i]] = initRandomPeakPop(getMuPerPeak());
        		}
        	}
        	dpiClustering.setNicheRadius(origNicheRad);
    		if (TRACE_DEMES) printDemes("After archivie-merge: ", peakPopSet);
        }
		
		if (TRACE) for (int k=0; k<peakPopSet.length-1; k++) {
...
//		nextGeneration = this.generateEvalChildren(dps); // create lambda new ones from mu parents
        for (int clustIndex=1; clustIndex<peakPopSet.length; clustIndex++) {
        	...
        	if (curSpecies.size()==1 && (!isAllowSingularPeakPops())) {
        		...
        		if (TRACE) System.out.println("Adding best from other species: " + bestOther);
			...
			if (doDraw(population.getGeneration())) drawPeakPop(""+clustIndex, curSpecies);
			if (TRACE) System.out.println("Optimizing cluster index " + (clustIndex) + ", size " + curSpecies.size());
...
//        	optimizedSpecies.incrFunctionCallsBy(optimizedSpecies.size());
        	if (TRACE) System.out.println("  ..." + optimizedSpecies.size() + " more funcalls... ");
        ...
        if (doEtaPreselection) { // this basically replaces ES-environment selection
        	...
        	for (int i=0; i<peakOpts.length; i++) {
        		...
        		if (i+1>=peakPopSet.length) {
        			...
        			if (TRACE) System.out.println("EtaPresel: from " + offspring.size() + " offspring selecting "+ eta);
        			...
        			if (delta>0) {
        				...
        				if (TRACE) System.out.println("Adding " + Math.min(delta, filterPeakPop.size()) + " from peak population.");
        				...
        				if (selected.size()<muPerPeak && (addLonersToPeaks )) {
        					...
        					if (TRACE) System.out.println("filling up with lucky loners: "+ delta);
        			...
        			if (TRACE) for (int k=0; k<i; k++) if (selected.setCut(peakOpts[k].getPopulation()).size()>2) { //one may happen after a cluster had size one (see above) 
            		...
            		if (doDraw(population.getGeneration())) drawPeakPop(""+i, selected);
        		}
        	}
        }
//        System.out.println("Best of second peak: " + clusteredPeakPops[2].getBestEAIndividual());
        ...
        if (getNumRndImmigrants()>0) generateEvalImmigrants(getNumRndImmigrants());
        collectPopulationIncGen(population, peakOpts, randomNewIndies);
...
//				System.out.println("Copied hash for peak " + clusteredPeakPops[i].getBestEAIndividual() + ", cluster " + i + " from " + origEsPop);
				...
				if (population.getGeneration()>1 && (getNumRndImmigrants()==0)) System.err.println("Error, empty original es pop ID!");
			}
		}
	}
...
//	}
	...
	private void printDemes(String prefix, Population[] peakPops) {
		...
		for (int i=0; i<peakPops.length; i++) {
			try {
				if (peakPops[i].size()>0) System.out.print(format(peakPops[i].getBestEAIndividual().getFitness(0), 3, 3));
			} catch(Exception e) {
	...
	public static String format(double d, int len, int prec) {
		StringBuffer sb = new StringBuffer();
		Formatter fm = new Formatter(sb); 
		if (Math.abs(d)>1e6) fm.format("%"+prec+"."+len+"e ", d);
		else fm.format("%"+prec+"."+len+"f ", d);
		return sb.toString();
	}
	...
	/**
	 * Generate additional pseudo peak populations (species) by either moving unclustered ones
	 * or by creating new ones randomly. If unclustered indies are to be used, there must
	 * be enough of them available.
	 * The new peaks are appended to the given cluster list and returned (as a new array instance).
	 * 
	 * @param origClusters the original clusters, where index 0 is the unclustered rest
	 * @param cntPerNewSpecies the number of indies to use per new peak population
	 * @param fromUnclusteredOrRandomly if true, use unclustered indies as new pops, otherwise create them randomly
	 * @return
	 */
    private Population[] generateMissingSpecies(Population[] origClusters, int cntPerNewSpecies, int newPops, boolean fromUnclusteredOrRandomly) {
    	if (newPops==0) return origClusters;
    	else {
    		...
    		for (int i=0; i<origClusters.length; i++) newClusters[i]=origClusters[i];
    		if (TRACE) System.out.println("Generated missing peak species...");
       		return newClusters;
    	}
	}

    /**
     * Initialize a new peak population with the given number of indies,
     * which are initialized randomly (using the problem instance) and assigned
     * a maximally bad fitness.
     * 
	...
	private void generateEvalImmigrants(int cnt) {
        if (cnt > 0) {
			...
			if (TRACE) System.out.println("evaluated immigrants: " + randomNewIndies.size());
        } else randomNewIndies = null;
	}
	...
	private Population[] getOptPops() {
		...
		for (int i=0; i<peakOpts.length; i++) pops[i] = peakOpts[i].getPopulation();
		return pops;
	}

	private void drawPeakPop(String prefix, Population npop) {
		if (npop!=null && (npop.size()>0)) try {
			if (plot == null) {
				plot = new TopoPlot("Niching-ES " + npop.getGeneration(), "x", "y");
				plot.setParams(50, 50);
				if (problem instanceof Interface2DBorderProblem) plot.setTopology((Interface2DBorderProblem)problem);
			}
...
//		plot.drawPopulation(prefix, npop);
	 ...
	 * Specifically, the original population of each dps-member is cloned and the method
	 * adaptAfterSelection is called which may modify the population. The vector of these
	 * new populations is returned.
	 * 
	 * @param collectedPop
	 * @param dps
	 * @param opts
	...
	/**
	 * Update the collected population pop from the ESes. The function calls
	 * of one last iteration are added up, so this method is expected to be 
	 * called exactly once per niching iteration.
	 * Immigrants are added as well - and the number of immigrants is added to the pop's
	 * function calls.
	 * 
	 * @param pop
	 * @param esses
...
//			else if (pop.getGeneration()!=pi.getGeneration()) System.err.println("Error, mismatching generation in collectPopulation");
		...
		pop.incrGeneration();
		pop.synchSize();
		if (TRACE) System.out.println("Collected " + pop.size() + " indies in pop.");
	}
	
	/**
	 * Calculate the dynamic population size, which is the number of individuals that 
	 * are currently "alive" in the peak set.
	 * This must be implemented in analogy to {@link #collectPopulationIncGen(Population, EvolutionStrategies[], Population)}
	 * 
	 * @return
	 */
	...
	public void setExpectedPeaks(int ep) {
		if (ep>0) expectedPeaks = ep;
		else System.err.println("Error, expecting positive number of peaks!");
	}
	...
	public void setExplorerPeaks(int ep) {
		if (ep>=0) explorerPeaks = ep;
		else System.err.println("Error, expecting nonzero number of explorer peaks!");
	}
    ...
    public InterfaceSolutionSet getAllSolutions() {
    	...
    	for (int i=0; i<peakOpts.length; i++) peaks.add(peakOpts[i].getPopulation().getBestEAIndividual());
    	if (archive!=null) peaks.addPopulation(archive); // add stored indies
    	peaks.synchSize();
...
//		int pSize = pop.size();
	...
	public void setResetExplorerInterval(int resInt) {
		if (resInt>0) this.resetExplorerInterval = resInt;
		else System.err.println("The explorer reset interval should be positive!");
	}
